...
import {
  compareWithOperationState,
  parseAdvancedOperationalReport
} from "../apps/api/src/services/reports/advanced-parser";

describe("parser avançado de relatórios operacionais", () => {
  it("separa múltiplas seções e preserva cabeçalho", () => {
    const parsed = parseAdvancedOperationalReport(
      "Setor: D1\nTurno: C\n🌱 Plantio de Cana\n1531/830 = rodando\n🚜 Preparo de Solo\n1601 = problema mecânico"
    );
    expect(parsed.header.shift).toBe("C");
    expect(parsed.sections.map((section) => section.operation)).toEqual([
      "Plantio Mecanizado",
      "Preparo de Solo"
    ]);
  ...
  it("cria alerta para implemento diferente e não altera automaticamente a coluna", () => {
    ...
    const comparison = compareWithOperationState(parsed.items, [
      { fleet: "1531", implement: "830", status: "RODANDO", description: "RODANDO" }
    ]);
...
@COA 🌱`);
    expect(parsed.header).toEqual({
      sector: "Rafael Pandolf",
      date: "10/07/2026",
      shift: "C",
      variety: "RB07-818"
    });
    expect(parsed.sections.map((section) => [section.operation, section.items.length])).toEqual([
      ["Plantio Mecanizado", 3],
      ["Colheita de Muda", 7]
    ]);
    expect(parsed.items.map((item) => item.mainEquipment)).toEqual([
      "1531",
      "1530",
      "1529",
      "625",
    ...
    expect(parsed.items.find((item) => item.mainEquipment === "1531")?.description).toBe(
      "DESLOCAMENTO PARA CHAPADINHA"
    );
    expect(parsed.items.find((item) => item.mainEquipment === "1509")?.informedImplement).toBe(
      "12010/1211"
    );
    expect(parsed.items.find((item) => item.mainEquipment === "626")?.forecastAt).toContain(
      "2026-07-10"
    );
  });
});
