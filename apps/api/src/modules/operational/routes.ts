...
type DemoEvent = Omit<OperationalEvent, "id" | "timestamp" | "simulated"> & { id?: string; timestamp?: string; simulated?: boolean };
...
export function operationalRoutes(prisma = new PrismaClient()) {
  ...
  router.get("/timeline", async (req, res, next) => { try { const ctx = await context(); res.json(ctx.engine.timeline({ since: req.query.since as string | undefined, operation: req.query.operation as string | undefined, fleet: req.query.fleet as string | undefined, shift: req.query.shift as string | undefined, group: req.query.group as string | undefined })); } catch (error) { next(error); } });
  router.post("/messages/parse", async (req, res, next) => { try { const ctx = await context(); const input = operationalParseSchema.safeParse(req.body); if (!input.success) throw new HttpError(400, "Mensagem inválida.", input.error.issues); res.json(ctx.workflow.parse(input.data.text, input.data)); } catch (error) { next(error); } });
  ...
  router.get("/messages", async (_req, res, next) => { try { const ctx = await context(); res.json(ctx.workflow.messages.list()); } catch (error) { next(error); } });
  router.get("/messages/:id", async (req, res, next) => { try { const ctx = await context(); const item = ctx.workflow.messages.find(String(req.params.id)); if (!item) throw new HttpError(404, "Mensagem não encontrada."); res.json(item); } catch (error) { next(error); } });
  ...
  router.get("/events", async (_req, res, next) => { try { const ctx = await context(); res.json(ctx.engine.timeline()); } catch (error) { next(error); } });
  router.get("/equipment/:fleet/history", async (req, res, next) => { try { const ctx = await context(); res.json(ctx.engine.fleetHistory(String(req.params.fleet))); } catch (error) { next(error); } });
  router.get("/equipment/:fleet/current-state", async (req, res, next) => { try { const ctx = await context(); res.json(ctx.engine.currentState(String(req.params.fleet))[0] ?? null); } catch (error) { next(error); } });
  router.get("/equipment/:fleet/downtime", async (req, res, next) => { try { const ctx = await context(); res.json(ctx.engine.stopMetrics(String(req.params.fleet))); } catch (error) { next(error); } });
  router.get("/pending", async (_req, res, next) => { try { const ctx = await context(); res.json(ctx.workflow.pendings.list()); } catch (error) { next(error); } });
  ...
  router.post("/projections/rebuild", async (_req, res, next) => { try { const ctx = await context(); res.json(await ctx.persistProjection()); } catch (error) { next(error); } });
  router.get("/shift-report/draft", async (_req, res, next) => { try { const ctx = await context(); const draft = ctx.workflow.shiftDraft(); await ctx.persistDraft(draft.text); res.json(draft); } catch (error) { next(error); } });
  router.get("/fleets/:fleet", async (req, res, next) => { try { const ctx = await context(); res.json(ctx.engine.fleetHistory(String(req.params.fleet))); } catch (error) { next(error); } });
  router.get("/shift-journal", async (req, res, next) => { try { const ctx = await context(); res.json(ctx.engine.shiftJournal(String(req.query.shift ?? "C"))); } catch (error) { next(error); } });
  router.get("/search", async (req, res, next) => { try { const ctx = await context(); res.json(ctx.engine.search(String(req.query.q ?? ""))); } catch (error) { next(error); } });
  router.post("/events", async (req, res, next) => {
    try {
      const ctx = await context();
      const event = ctx.engine.append({ ...req.body, type: req.body.type as OperationalEventType, source: "PANEL", approved: Boolean(req.body.approved), priority: req.body.priority ?? "normal", responsible: res.locals.user?.name ?? "Operador", user: res.locals.user?.name ?? "Operador" });
      await ctx.persistEvent(event);
      await ctx.persistProjection();
      res.status(201).json({ event, excelCommands: ctx.engine.toExcelCommands(event), whatsAppCommands: ctx.engine.toWhatsAppCommands(event), externalActionExecuted: false });
    } catch (error) { next(error); }
  });
  router.post("/pendencies/:id/action", async (req, res, next) => {
    try {
      ...
      if (item && req.body.action === "approve") result = ctx instanceof OperationalPrismaContext ? await ctx.approvePending(item.id, res.locals.user?.name ?? "Operador") : ctx.workflow.approve(item.id, res.locals.user?.name ?? "Operador");
        ...
        result = { simulated: true, externalActionExecuted: false, item, event, action: req.body.action };
      } else {
        const event = ctx.engine.append({ type: req.body.action === "approve" ? "PENDING_APPROVED" : "CONFIRMED", source: "PANEL", approved: true, priority: "normal", observation: `Pendência ${req.body.action}`, responsible: res.locals.user?.name ?? "Operador", user: res.locals.user?.name ?? "Operador" });
  ...
  router.post("/send-preview/confirm", async (req, res, next) => { try { const ctx = await context(); const event = ctx.engine.append({ type: "REPORT_SENT", source: "PANEL", approved: true, priority: "normal", operation: "Plantio Mecanizado", group: req.body.destination, observation: req.body.legend, shift: "C", user: res.locals.user?.name ?? "Operador", responsible: res.locals.user?.name ?? "Operador" }); await ctx.persistEvent(event); res.json({ simulated: true, externalActionExecuted: false, status: "PROCESSED", event, destination: req.body.destination, legend: req.body.legend, simulatedAt: event.timestamp }); } catch (error) { next(error); } });
  router.get("/shift-report", async (_req, res, next) => { try { const ctx = await context(); const draft = ctx.workflow.reports.getDraft(); const text = draft.includes("RELATÓRIO DE TROCA DE TURNO") ? draft : "*📋 RELATÓRIO DE TROCA DE TURNO*\n\nSem rascunho persistido."; res.json({ status: "DRAFT", journal: ctx.engine.shiftJournal("C"), text }); } catch (error) { next(error); } });
  router.post("/shift-report", async (req, res, next) => { try { const ctx = await context(); const event = ctx.engine.append({ type: req.body.action === "confirm" ? "SHIFT_REPORT_SENT" : "SHIFT_REPORT_CREATED", source: "PANEL", approved: true, priority: "normal", observation: "Relatório de troca de turno", shift: "C", user: res.locals.user?.name ?? "Operador", responsible: res.locals.user?.name ?? "Operador" }); ctx.workflow.reports.saveDraft(String(req.body.text ?? "")); await ctx.persistDraft(String(req.body.text ?? "")); await ctx.persistEvent(event); res.json({ simulated: true, externalActionExecuted: false, status: req.body.action === "confirm" ? "CONFIRMED" : "DRAFT", text: req.body.text, event, savedAt: event.timestamp }); } catch (error) { next(error); } });
  return router;
}

function eventIds(ctx: OperationalContext) { return new Set(ctx.engine.all().map(event => event.id)); }
function pendingSummary(ctx: OperationalContext) {
  const persisted = ctx.workflow.pendings.list().map(item => ({ id: item.id, operation: item.interpretation.operation ?? "Não informada", subject: `Frota ${item.interpretation.mainEquipment ?? "sem frota"}`, reason: item.reason === "DOUBT" ? "Revisão necessária" : "Alteração aguardando aprovação", since: item.createdAt, priority: item.interpretation.confidence < 0.6 ? "urgent" : "high", status: item.status === "OPEN" ? "open" : "resolved" }));
...
function demoSeed(): DemoEvent[] {
  ...
  return [
    { ...common, id: "demo-shift-started", type: "SHIFT_STARTED", timestamp: ago(120), observation: "Turno C iniciado" },
    { ...common, id: "demo-stop-626", type: "STOPPED", timestamp: ago(42), fleet: "626", operation: "Plantio Mecanizado", newStatus: "PARADO", newDescription: "Cilindro de inclinação quebrado", newSector: "D1", priority: "high" },
    { ...common, id: "demo-stop-1530", type: "STOPPED", timestamp: ago(31), fleet: "1530/2006", implement: "2006", operation: "Colheita de Muda", newStatus: "PARADO", newDescription: "Atolada", newSector: "Chapadinha", priority: "high" },
    { ...common, id: "demo-message-625", type: "MESSAGE_RECEIVED", timestamp: ago(19), fleet: "625", operation: "Plantio Mecanizado", group: "Plantio / Muda / Preparo", originalMessage: "Frota 625 parada, bico injetor e fumaça preta", observation: "Mensagem recebida" },
    { ...common, id: "demo-pending-625", type: "PENDING_CREATED", timestamp: ago(18), fleet: "625", operation: "Plantio Mecanizado", group: "Plantio / Muda / Preparo", newStatus: "PARADO", newDescription: "Bico injetor / fumaça preta", newSector: "D1", approved: false, priority: "urgent" },
    { ...common, id: "demo-stop-625", type: "STOPPED", timestamp: ago(18), fleet: "625", operation: "Plantio Mecanizado", newStatus: "PARADO", newDescription: "Bico injetor / fumaça preta", newSector: "D1", priority: "urgent" },
    { ...common, id: "demo-displaced-1531", type: "DISPLACED", timestamp: ago(12), fleet: "1531/830", implement: "830", operation: "Colheita de Muda", newStatus: "DESLOCAMENTO", newDescription: "Deslocamento para Chapadinha", newSector: "Estrada" },
    { ...common, id: "demo-status-1601", type: "STATUS_CHANGED", timestamp: ago(8), fleet: "1601", operation: "Preparo de Solo", newStatus: "DISPONIVEL", newDescription: "Disponível", newSector: "J2" },
    { ...common, id: "demo-error-cpd", type: "ERROR", timestamp: ago(4), operation: "CPD", group: "CPD", originalMessage: "Informação sem identificação da frota", observation: "Falha de processamento", priority: "high" }
  ];
}
