...
type RealSnapshot = {
  fleets: RealFleetState[];
  readAt: string;
  source: "EXCEL_COM_LOCAL_DEV";
  unavailableSheets: string[];
};
let cachedSnapshot: RealSnapshot | undefined;
let refreshPromise: Promise<RealSnapshot> | undefined;
const snapshotTtlMs = 30_000;

export function invalidateRealLocalFleetSnapshot() {
  cachedSnapshot = undefined;
}

export async function readRealLocalFleetSnapshot(prisma: PrismaClient) {
  if (cachedSnapshot && Date.now() - new Date(cachedSnapshot.readAt).getTime() < snapshotTtlMs)
...
function refreshSnapshot(prisma: PrismaClient) {
  if (!refreshPromise)
    refreshPromise = loadSnapshot(prisma)
      .then((result) => (cachedSnapshot = result))
      .finally(() => {
...
async function loadSnapshot(prisma: PrismaClient): Promise<RealSnapshot> {
  ...
  const groups = new Map<
    string,
    { definition: ReturnType<typeof localWorkbookByOperation>; operations: typeof operations }
  >();

  for (const operation of operations) {
    if (!operation.sheetName) continue;
    const definition = localWorkbookByOperation(operation.name);
    const group = groups.get(definition.id) ?? { definition, operations: [] };
    group.operations.push(operation);
    groups.set(definition.id, group);
  }

  for (const group of groups.values()) {
    const workbook = validateLocalOperationalWorkbook({
    ...
    const command: ExcelAgentCommand = {
      ...
      requestedAt: readAt,
      requestedBy: "operational-snapshot",
      type: "READ_OPERATION_SHEETS",
      workbook: workbook.filePath,
      payload: {
        sheets: group.operations.map((operation) => ({
          operation: operation.name,
          worksheet: resolveSheetName(operation.name, operation.sheetName!),
          range: "A1:S250"
        }))
      },
      simulation: false,
    ...
    const response = await waitForExcelResult(command.commandId, 65_000);
    const sheets = response.success
      ? ((
          response.result as {
            sheets?: Array<{
              operation: string;
    ...
    for (const operation of group.operations) {
      ...
      const result = sheets.find((item) => item.operation === operation.name);
      if (!result?.success) {
        unavailableSheets.push(`${operation.name}: ${worksheet}`);
        continue;
      }
      ...
      for (const row of values) {
        ...
        fleets.push({
          ...
          stoppedMinutes,
          stopMetrics: {
            todayMinutes: stoppedMinutes,
            shiftMinutes: stoppedMinutes,
            weekMinutes: stoppedMinutes,
            stopCount: startedAt ? 1 : 0,
            longestMinutes: stoppedMinutes,
            lastStopAt: startedAt?.toISOString(),
