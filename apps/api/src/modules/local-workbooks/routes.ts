...
const configuredRoot =
  process.env.LOCAL_OPERATIONAL_WORKBOOK_ROOT?.trim() || "planilhas-homologacao";
const root = path.resolve(
  path.isAbsolute(configuredRoot) ? configuredRoot : path.join(projectRoot, configuredRoot)
);
...
export function localWorkbookRoutes(prisma = new PrismaClient()) {
  ...
  router.get("/", (_req, res) => {
    res.json({
      ...
      localOperationalExcelWrite: process.env.LOCAL_OPERATIONAL_EXCEL_WRITE === "true",
  ...
  router.get("/status", (_req, res) => {
    res.json({
      mode: "LOCAL_OPERATIONAL",
      root: root.replace(process.cwd(), "<project>"),
      workbooks: listWorkbooks(),
      officialExcelWrite: false
    });
  });
  ...
  router.post("/:id/open", async (req, res, next) => {
    try {
      res.json(await command("OPEN_LOCAL_WORKBOOK", workbookById(req.params.id)));
    } catch (error) {
      next(error);
    }
  });
  ...
  router.post("/open-all", async (_req, res, next) => {
    try {
      const results = [];
      for (const workbook of LOCAL_OPERATIONAL_WORKBOOKS)
        results.push(await command("OPEN_LOCAL_WORKBOOK", workbookById(workbook.id)));
      res.json({ results });
    } catch (error) {
      next(error);
    }
  ...
  router.post("/open-folder", async (_req, res, next) => {
    try {
      res.json(await command("OPEN_LOCAL_FOLDER"));
    } catch (error) {
      next(error);
    }
  });
  ...
  router.post("/preview/:pendingId", async (req, res, next) => {
    try {
      res.json(await buildLocalPreview(prisma, String(req.params.pendingId)));
    } catch (error) {
      next(error);
    }
  });
  ...
  router.post("/apply/:pendingId", async (req, res, next) => {
    try {
      ...
      if (setting?.value !== "LOCAL_OPERATIONAL")
        throw new HttpError(409, "Ative LOCAL_OPERATIONAL antes da escrita local.", {
          code: "LOCAL_OPERATIONAL_MODE_REQUIRED"
        });
      if (process.env.OFFICIAL_EXCEL_WRITE === "true")
        throw new HttpError(409, "OFFICIAL_EXCEL_WRITE precisa permanecer false.", {
          code: "OFFICIAL_EXCEL_WRITE_MUST_STAY_FALSE"
        });
      if (process.env.LOCAL_OPERATIONAL_EXCEL_WRITE !== "true")
        throw new HttpError(409, "LOCAL_OPERATIONAL_EXCEL_WRITE=true é obrigatório.", {
      ...
      if (!result.success)
        throw new HttpError(409, "Escrita local bloqueada pelo Excel Agent.", {
      ...
      await prisma.systemLog.create({
        data: {
          ...
          action: "LOCAL_OPERATIONAL_EXCEL_WRITE",
          entity: "PendingChange",
          entityId: preview.pendingId,
          message: "Alteração local .dev confirmada, escrita e relida pelo Excel Agent.",
          metadata: {
            backup,
      ...
      await prisma.pendingChange.update({
        where: { id: preview.pendingId },
        data: { active: false, status: "APPROVED_SIMULATED", updatedAt: new Date() }
      });
  ...
  router.post("/image/:pendingId", async (req, res, next) => {
    try {
      ...
      const range =
        preview.operation === "Plantio Mecanizado" ? "B2:S20" : operation?.imageRange || "A1:AI26";
      const outputDir = path.join(tempRoot, new Date().toISOString().slice(0, 10));
      fs.mkdirSync(outputDir, { recursive: true });
      const tempPath = path.join(
        outputDir,
        `${preview.fleet.replace(/[^a-z0-9-]/gi, "_")}-${Date.now()}.png`
      );
      const excelCommand: ExcelAgentCommand = {
        commandId: randomUUID(),
        correlationId: randomUUID(),
      ...
      if (!result.success)
        throw new HttpError(409, "CopyPicture local falhou.", {
          code: result.error?.code,
          message: result.error?.message
        });
      res.json({
        image: { path: tempPath.replace(projectRoot, "<project>"), range },
        result,
        sendMessage: false,
        sendReaction: false
      });
...
function listWorkbooks() {
  return LOCAL_OPERATIONAL_WORKBOOKS.map((definition) => {
    const validation = validateLocalOperationalWorkbook({
      id: definition.id,
      root,
      officialRoot,
      mustExist: false
    ...
    return {
      ...
      path: maskLocalWorkbookPath(validation.filePath, root),
      absolutePath: undefined,
      writable: process.env.LOCAL_OPERATIONAL_EXCEL_WRITE === "true",
      status: "CLOSED",
      officialExcelWrite: false
    };
  });
...
async function command(
  ...
  workbook?: { absolutePath?: string },
  options: { worksheet?: string; payload?: Record<string, unknown> } = {}
) {
  const excelCommand: ExcelAgentCommand = {
    commandId: randomUUID(),
    correlationId: randomUUID(),
...
async function buildLocalPreview(prisma: PrismaClient, pendingId: string) {
  const pending = await prisma.pendingChange.findUnique({
    where: { id: pendingId },
    include: { operation: true, incomingMessage: { include: { parsedMessages: true } } }
  });
  ...
  const worksheet = resolveWorksheet(
    ...
    pending.operation?.sheetName ||
      (pending.operation?.name === "Plantio Mecanizado" ? "PLANTIO" : "CPD")
  );
  const findResult = await command("FIND_EQUIPMENT", workbook, {
    worksheet,
    payload: {
      fleetColumn: pending.operation?.fleetColumn || "F",
      fleet: pending.equipmentCode,
      headerRow: 7
    }
  });
  const match = (
    findResult.result as { candidates?: Array<{ row: number }>; status?: string } | undefined
  )?.candidates?.[0];
  if (
    !findResult.success ||
    (findResult.result as { status?: string } | undefined)?.status !== "UNIQUE_MATCH" ||
    !match
  )
    throw new HttpError(409, "Frota não localizada de forma única na planilha .dev.", {
      code: "AMBIGUOUS_MATCH"
    });
  const row = Number(match.row);
  const parsed =
    pending.incomingMessage?.parsedMessages
      ?.map((item) => item.parsedJson as Record<string, unknown>)
      .find((item) => String(item.mainEquipment) === pending.equipmentCode) ?? {};
  ...
  const statusColumn = pending.operation?.statusColumn || "H";
  const descriptionColumn = pending.operation?.descriptionColumn || "S";
  const cells = {
    statusCell: `${statusColumn}${row}`,
    startDateCell: `K${row}`,
    startTimeCell: `L${row}`,
    forecastDateCell: `M${row}`,
    forecastTimeCell: `N${row}`,
    descriptionCell: `${descriptionColumn}${row}`
  };
  const preview = await command("PREVIEW_CHANGE", workbook, {
    worksheet,
    payload: { ...cells, proposed, mappingConfirmed: false }
  });
  if (!preview.success)
    throw new HttpError(409, "Leitura da prévia local falhou.", { code: preview.error?.code });
  const current =
    (preview.result as { current?: Record<string, { value?: unknown }> } | undefined)?.current ??
    {};
  return {
    mode: "LOCAL_OPERATIONAL",
    officialExcelWrite: false,
    ...
    workbook: {
      ...
      path: maskLocalWorkbookPath(workbook.filePath, root),
      absolutePath: workbook.filePath
    },
    worksheet,
    fleet: pending.equipmentCode,
...
function resolveWorksheet(operation: string, configured: string) {
...
function normalizeCurrent(current: Record<string, { value?: unknown }>) {
  return {
    status: current.statusCell?.value ?? "",
    startDate: current.startDateCell?.value ?? "",
    startTime: current.startTimeCell?.value ?? "",
    forecastDate: current.forecastDateCell?.value ?? "",
    forecastTime: current.forecastTimeCell?.value ?? "",
    description: current.descriptionCell?.value ?? ""
  };
}
...
function plannedBackup(filePath: string) {
  ...
  return {
    path: path.join(folder, name).replace(projectRoot, "<project>"),
    folder: folder.replace(projectRoot, "<project>")
  };
}
...
function createLocalBackup(filePath: string, pendingId: string, fleet: string, operator: string) {
  ...
  if (fs.existsSync(backupPath))
    throw new HttpError(409, "Backup local já existe; nova tentativa necessária.", {
      code: "LOCAL_BACKUP_ALREADY_EXISTS"
    });
  fs.mkdirSync(path.dirname(backupPath), { recursive: true });
  fs.copyFileSync(filePath, backupPath, fs.constants.COPYFILE_EXCL);
  const hash = hashFileIfExists(backupPath);
  if (!hash)
    throw new HttpError(409, "Backup local não pôde ser validado por hash.", {
      code: "LOCAL_BACKUP_HASH_FAILED"
    });
  const sizeBytes = fs.statSync(backupPath).size;
  return {
    path: backupPath.replace(projectRoot, "<project>"),
    hash,
    sizeBytes,
